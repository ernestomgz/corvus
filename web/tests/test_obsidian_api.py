...
def test_obsidian_preview_imports_question_source_note_manifest(api_client, user_factory, deck_factory):
    user = user_factory()
    root = deck_factory(user=user, name='STEM', parent=None)
    api_client.force_login(user)
    ...
    main_content = (
        '---\n'
        'questions-source: "[[Questions Calculus]]"\n'
        'card-tags:\n'
        '  - calculus\n'
        '---\n'
        'Main front\n'
    ...
    payload = {
        'source_path': 'Math/Calculus.md',
        'root_deck_path': 'STEM',
        'source_hash': 'hash-with-question-source',
        'note_manifest': json.dumps(
            [
                {'field': 'note_0', 'path': 'Math/Calculus.md'},
                {'field': 'note_1', 'path': 'Math/Questions/Questions Calculus.md'},
            ]
        ),
        'note_0': SimpleUploadedFile('Calculus.md', main_content.encode('utf-8'), content_type='text/markdown'),
        'note_1': SimpleUploadedFile(
            'Questions Calculus.md',
            question_content.encode('utf-8'),
            content_type='text/markdown',
        ),
    }

    preview_response = api_client.post('/api/v1/obsidian/preview', data=payload)
    assert preview_response.status_code == 201
    preview_data = preview_response.json()
    assert len(preview_data['cards']) == 2
    assert {card['source_path'] for card in preview_data['cards']} == {
        'Math/Calculus.md',
        'Math/Questions/Questions Calculus.md',
    }
    ...
    apply_response = api_client.post(
        ...
        data=json.dumps(
            {
                'source_hash': 'hash-with-question-source',
                'decisions': [{'index': 0, 'action': 'apply'}, {'index': 1, 'action': 'apply'}],
            }
        ),
    ...
    assert apply_data['summary']['created'] == 2
    assert {card['source_path'] for card in apply_data['cards']} == {
        'Math/Calculus.md',
        'Math/Questions/Questions Calculus.md',
    }

    cards_by_source = {card.source_path: card for card in Card.objects.filter(user=user)}
    assert cards_by_source['Math/Calculus.md'].deck == Deck.objects.get(user=user, parent=root, name='Math')
    assert cards_by_source['Math/Calculus.md'].tags == ['calculus']
    assert cards_by_source['Math/Questions/Questions Calculus.md'].tags == ['calculus', 'exam']
