...
# ============================================================================
# REGRESSION: Multiple Blank Lines After Cards Should Not Create Ghost Cards
# ============================================================================

def test_long_card_followed_by_double_blank_and_text_no_ghost_card(user_factory, deck_factory):
    """
    Regression test: Ensure that text after double blank lines following a long-card
    does NOT create a spurious card. Long-cards allow single blank lines but terminate on double.
    Issue found in samples/import_fields.md lines 164-184.
    """
    user = user_factory()
    deck = deck_factory(user=user)
    markdown = (
        ...
        "\n"
        "\n"  # Two blank lines terminate long-card
        "External text after the card.\n"
        "Should not be in any card."
    )
    archive = _build_zip({'notes.md': markdown})
    record = process_markdown_archive(user=user, deck=deck, uploaded_file=archive)
    
    # Should create exactly ONE card, not two
    assert record.summary['created'] == 1
    card = Card.objects.get(user=user)
    
    # Card should include both paragraphs (single blank line allowed in long-card)
    assert 'First line of content' in card.back_md
    assert 'Second paragraph' in card.back_md
    
    # Card MUST NOT include text after double blank lines
    assert 'External text' not in card.back_md
    assert 'Should not be' not in card.back_md


def test_short_card_terminated_by_single_blank_line(user_factory, deck_factory):
    """
    Regression test: Short cards (#card) terminate on a SINGLE blank line.
    Do not continue to next paragraph. Text after blank line should not be included.
    """
    user = user_factory()
    deck = deck_factory(user=user)
    markdown = (
        ...
        "\n"
        "More external text.\n"
        "Still not part of card."
    )
    archive = _build_zip({'notes.md': markdown})
    record = process_markdown_archive(user=user, deck=deck, uploaded_file=archive)
    
    # Should create exactly ONE card
    assert record.summary['created'] == 1
    card = Card.objects.get(user=user)
    ...
    assert 'This should not' not in card.back_md
    assert 'External content' not in card.back_md
    assert 'More external' not in card.back_md


def test_long_card_with_code_block_double_blank_after_no_ghost(user_factory, deck_factory):
    """
    Regression test: Long card with code block. Single blank lines inside are allowed.
    Double blank lines terminate card. Text after should not create a ghost card.
    """
    user = user_factory()
    deck = deck_factory(user=user)
    markdown = (
        ...
        "External text after card.\n"
        "Not part of card."
    )
    archive = _build_zip({'notes.md': markdown})
    record = process_markdown_archive(user=user, deck=deck, uploaded_file=archive)
    
    # Should create only ONE card
    assert record.summary['created'] == 1
    card = Card.objects.get(user=user)
    
    # Card should include all content before double blank
    assert 'Intro line' in card.back_md
    assert '```' in card.back_md
    assert 'code line 1' in card.back_md
    assert 'Closing line' in card.back_md
    
    # Should NOT include content after double blank
    assert 'External text' not in card.back_md
    assert 'Not part' not in card.back_md


def test_multiple_short_cards_separated_by_blanks_no_ghost_cards(user_factory, deck_factory):
    """
    Test multiple short cards separated by blank lines.
    Ensure no ghost cards are created in the gaps.
    """
    user = user_factory()
    deck = deck_factory(user=user)
    markdown = (
    ...
    # Should create exactly THREE cards, not more
    assert record.summary['created'] == 3
    cards = Card.objects.filter(user=user).order_by('created_at')
    
    assert 'Text 1' in cards[0].front_md
    assert 'Back 1' in cards[0].back_md
    
    assert 'Text 2' in cards[1].front_md
    assert 'Back 2' in cards[1].back_md
    
    assert 'Text 3' in cards[2].front_md
    assert 'Back 3' in cards[2].back_md


def test_long_card_with_many_trailing_blanks_and_text_no_ghost(user_factory, deck_factory):
    """
    Edge case: Long card followed by many consecutive blank lines and then text.
    Should not create ghost card.
    """
    user = user_factory()
    deck = deck_factory(user=user)
    markdown = (
    ...
    assert 'Paragraph 1' in card.back_md
    assert 'Paragraph 2' in card.back_md
    assert 'Text after blanks' not in card.back_md
    assert 'Not in card' not in card.back_md


def test_short_card_same_line_marker_terminates_on_blank(user_factory, deck_factory):
    """
    Test same-line short card marker (#card on same line as heading).
    Short cards terminate on single blank line regardless of marker position.
    """
    user = user_factory()
    deck = deck_factory(user=user)
    markdown = (
        "## Heading Text #card\n"
        "Back content line.\n"
        "\n"  # Single blank terminates short card
        "External text not in card.\n"
        "Also not in card."
    )
    archive = _build_zip({'notes.md': markdown})
    record = process_markdown_archive(user=user, deck=deck, uploaded_file=archive)
    ...
    assert 'Heading Text' in card.front_md
    assert 'Back content line' in card.back_md
    
    # External text should not be part of the card
    assert 'External text' not in card.back_md
    assert 'Also not' not in card.back_md


def test_mixed_long_and_short_cards_different_blank_line_behavior(user_factory, deck_factory):
    """
    Test mixed document with long-card and short card showing different behavior:
    - Long-card allows single blank lines, terminates on double blank
    - Short card terminates on single blank line
    No ghost cards should be created.
    """
    user = user_factory()
    deck = deck_factory(user=user)
    markdown = (
        ...
        "Long card line 2\n"
        "\n"
        "\n"  # Double blank terminates long-card
        "External text 1.\n"
        "Not in card.\n"
        "\n"
        "\n"
    ...
    # Should create exactly TWO cards, no ghost cards
    assert record.summary['created'] == 2
    cards = Card.objects.filter(user=user).order_by('created_at')
    
    # First card: long-card with both lines (single blank allowed)
    assert 'Long card line 1' in cards[0].back_md
    assert 'Long card line 2' in cards[0].back_md
    assert 'External text 1' not in cards[0].back_md
